...
export function user_auth_action( user_infos ){
    ...
    // Login a USER via wordpress API
    return function (dispatch) {
        return axios.post(`${WORDPRESS_API_BASE_URL}/jwt-auth/v1/token`,{
            ...
            // Avoir l'ID dans la response
            const uid = response.data.user_id;
            const auth_token = response.data.token;

            // Retrieve le USER
            // Vérifier si l'USER est valide

            // REQUEST AVEC AUTH
            axios.get(`${WORDPRESS_API_BASE_URL}/wp/v2/users/${uid}`,{
                ...
                .then(function (response) {
                    ...
                    response.data.acf.user_validation === true ?
                        isValidated = true :
                        isValidated = false
